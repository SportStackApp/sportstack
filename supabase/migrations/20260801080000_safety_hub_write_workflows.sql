-- Atomic write entrypoint for the existing Safety Hub forms. SECURITY INVOKER
-- deliberately preserves the table RLS policies and audit triggers.
    ...
    when 'risk' then
      if p_record_id is null then
        insert into public.rg_risk_register (
          ...
          p_association_id, p_club_id, p_team_id, btrim(p_payload ->> 'title'),
    ...
    when 'action' then
      if p_record_id is null then
        insert into public.rg_be_smart_actions (
          ...
          p_association_id, p_club_id, p_team_id, v_risk_id,
