-- Qualify profile_claim_reviews columns that share names with the function's
-- output fields. Without the table alias, PL/pgSQL treats them as ambiguous.
...
BEGIN
  ...
  SELECT
    count(*),
    min(r.id::text)::uuid,
    min(r.placeholder_profile_id::text)::uuid,
    min(r.match_value)
