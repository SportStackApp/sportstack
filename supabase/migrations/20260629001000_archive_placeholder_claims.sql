-- Replace the initial placeholder claim helper with an archive-first flow and
-- durable review/audit records. This keeps merge actions additive and avoids
-- deleting placeholder profile/Auth records during automatic login claims.

CREATE TABLE IF NOT EXISTS public.profile_claim_reviews (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  real_profile_id uuid REFERENCES public.profiles(id) ON DELETE CASCADE,
  placeholder_profile_id uuid REFERENCES public.profiles(id) ON DELETE SET NULL,
  status text NOT NULL DEFAULT 'needs_admin_review' CHECK (status IN ('needs_admin_review', 'approved', 'rejected', 'merged')),
  match_method text NOT NULL CHECK (match_method IN ('revsports_player_id', 'admin_approved')),
  match_value text,
  reason text,
...
CREATE INDEX IF NOT EXISTS profile_claim_reviews_real_status_idx
  ON public.profile_claim_reviews (real_profile_id, status);

CREATE INDEX IF NOT EXISTS profile_claim_reviews_placeholder_status_idx
  ON public.profile_claim_reviews (placeholder_profile_id, status);

CREATE UNIQUE INDEX IF NOT EXISTS profile_claim_reviews_open_unique_idx
  ON public.profile_claim_reviews (real_profile_id, placeholder_profile_id, match_method)
  WHERE status = 'needs_admin_review';

CREATE TABLE IF NOT EXISTS public.profile_claim_audit (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  real_profile_id uuid REFERENCES public.profiles(id) ON DELETE SET NULL,
  placeholder_profile_id uuid REFERENCES public.profiles(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS profile_claim_audit_real_created_idx
  ON public.profile_claim_audit (real_profile_id, created_at DESC);
...
BEGIN
  ...
  SELECT count(*), min(id), min(placeholder_profile_id), min(match_value)
