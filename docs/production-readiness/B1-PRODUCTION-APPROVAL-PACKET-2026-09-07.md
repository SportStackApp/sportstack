...
the hosted rehearsal passed. Aaron confirmed that only Super Admin may add or remove account-wide
Player/Voter roles; that rule is now enforced and retested. Production approval remains blocked
until Aaron completes the B1c walkthrough, confirms the Primary-team semantics, and Production
access supports a fresh read-only drift check and verified backup.
...
| Candidate | `a076174f317a4fbdc66c50e312b7257624ad33b0` |
| Candidate branch | `fix/b1-global-simple-role-boundary` |
| Changed files | 28 |
| Binary patch SHA-256 | `c58dc1ed7020f0cf67806f33bc58b6450bd9b4be2f9bf44bc30e54c2db0293af` |
| Vercel preview | `GYqrVj6kNfD9y4qWsbp9JGByZYAR` — SUCCESS |

The package contains the scoped permission/session application changes and the seven migrations
...
2. Fetch all branches and confirm Production is still exactly `a1d23c7`, the candidate is exactly
   `a076174`, and the candidate is a direct descendant of Production.
...
4. Capture the current migration list, schema fingerprint, grants, policies, scheduled jobs and
   protected row counts. The seven B1 versions must be absent.
...
If Production still has 159 versions, the expected final count is 166. The foundation adds 12 tables
...
4. Re-run the B1 static checks, function grants, anonymous denials, actual-role boundaries and
   database advisers.
5. Fast-forward only the Production branch from `a1d23c7` to `a076174`. Do not merge Main or Dev.
...
`RELEASE B1 ACCESS PACKAGE a076174 TO PRODUCTION`
