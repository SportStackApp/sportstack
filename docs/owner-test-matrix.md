...
| Fixture list/calendar buttons visibly switch views | Calendar view now has previous/next/current-month navigation, keeps historical fixtures available and uses a muted historical colour distinct from upcoming fixtures. The selected month is URL-backed. | Implemented in Dev batch — deployed retest required |
