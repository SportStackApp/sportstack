...
| Communications | Team Chat, replies, reactions and removed placeholders loaded. Commit `a77f01a` adds account/channel draft persistence across full reload, explicit legacy-history wording and tested 50-message merge/pagination helpers. Three focused tests cover edits, a 51-message page and full-page detection. Aaron confirmed an unsent Team Chat draft survives `Ctrl + Shift + R`; no message was published for that test. | **Owner draft-reload pass plus automated pagination pass — 10/08/2026** |
