...
| Scheduled fixture detail opens for actual Team Manager | On deployed `f3486b0`, the Pumas upcoming fixture list loaded correctly but opening a scheduled fixture hit the route error boundary. `GameDetail` rendered status `MAYBE` through a style map that incorrectly used `UNSURE`; it also used legacy `PENDING` instead of generated enum value `NO_RESPONSE`. The local repair aligns all four enum values and type-checks the complete style map. | **Fail reproduced; fix prepared — deploy and owner retest required** |
