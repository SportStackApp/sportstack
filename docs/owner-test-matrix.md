...
| Communications | Team Chat, replies, reactions and removed placeholders loaded. Commit `a77f01a` adds account/channel draft persistence across full reload, explicit legacy-history wording and tested 50-message merge/pagination helpers. Three focused tests cover edits, a 51-message page and full-page detection. Aaron confirmed an unsent Team Chat draft survives `Ctrl + Shift + R`; no message was published for that test. He also confirmed an edited message history shows the current and earlier versions with editor/timestamp details. | **Owner draft/history pass plus automated pagination pass — 10/08/2026** |
...
| All assigned teams appear, including Lucas HC | The failed nested role query was replaced in `a77f01a` with a flat role query resolved through complete TeamContext team/club/association data. Aaron then confirmed the actual AM Player mode offers Primary Pumas and Secondary Lucas HC; Lucas loaded and remained selected without bouncing back. | **Actual Player multi-team owner pass — 10/08/2026. Multi-Team-Manager-role variant remains pending.** |
