...
| Fixtures and byes | Fixtures shows the full competition name, `Round 15` and `Lucas HC — Bye` without midnight/TBD. The current My Dashboard formatter now uses the same bye wording and suppresses time/venue details for byes; a deployed Lucas HC browser retest remains. The Fixtures calendar button only changes the existing cards to a two-column grid; it does not render a calendar. | **Partial pass — bye fix present; calendar behaviour remains** |
...
| Byes display as `Team — Bye` without Unknown, midnight or TBD | Fixtures is correct; the current My Dashboard formatter also uses `Team — Bye` and suppresses time/venue details for byes | Dev present — Lucas HC retest |
...
| Profile shows active mode plus all assigned roles and scopes, not only Voter | The Profile badge now reads the confirmed active mode while the separate role badges retain every assigned role and scope. Actual Player profile showed Player mode and Pumas scope. | Fix prepared; multi-role deployed retest pending |
