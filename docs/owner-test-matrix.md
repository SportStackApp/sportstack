...
| Fixtures and byes | On deployed build `b03938d`, the Team Manager fixture list showed Lucas HC fixtures only, the full competition name, full `Round 15` wording and `Lucas HC — Bye` without time or venue. A shared display repair is now prepared for Division/Team Overviews, individual fixture detail and Admin Fixtures. The Fixtures calendar button previously only changed the existing cards to a two-column grid; a fresh control retest remains. | **Team fixture list pass; shared bye repair awaiting deployed retest; calendar retest remains** |
...
| Umpire Match Voting | Guided Round → Division → Fixture selection, round dates, current Round 12, completed-fixture eligibility, division scheme, inline missing-value validation, number-only warning/acknowledgement and one-character search all worked. An actual disposable Umpire submitted a three-line ballot successfully after the fixture-scope helper was repaired. A second submission for the same Umpire and fixture returned the clear duplicate-ballot error. The exact test submission was then soft-deleted, retained in the audit log and verified absent from active submissions. Source review still confirms the broad suggestions come from every active membership across every team in both fixture clubs and then infer a fixture side by club, rather than limiting suggestions to the fixture roster plus the two fixture teams. | **Submission and duplicate protection pass — suggestion scope defect remains** |
...
| Dev integrity and quality | All relevant inspected tables have RLS. Live counts remain 201 duplicate active membership groups, 44 multiple-Primary users and the 490-row immutable snapshot. `npm run lint:dev-plan`, `npx tsc --noEmit`, `npm run build` and all 125 Python tests passed. A continuation on commit `4390b47` also re-ran 32 focused session-context, voting-module, Committee/Safety and SQL-safety `unittest` checks; all passed. Full lint remains at its known 362-error/76-warning baseline. | **Automated pass; cleanup approval-gated** |
...
| Selecting a club or division must not predictively select a lower cascade level | On deployed build `b03938d`, selecting Lucas HC and then Division 1 Open automatically selected the Lucas HC team while leaving the user on Division Overview. This makes the selected scope look like Team Overview even though the page remains division-wide. Parent changes should clear lower levels and require an explicit team selection. | Change logged — defer implementation |
...
| Owner observation or requirement | Current position | Test status |
|---|---|---|
| Admin Fixtures Management must clearly reflect the active cascade scope | On deployed build `b03938d`, Lucas HC scope returned 15 apparently team-relevant fixtures, but the page filters still displayed `All associations`, `All clubs`, `All divisions` and `All teams`. A prepared UI repair changes Home/Away columns to team-only names, removes the fake `Round date` time and displays a past bye as Completed without rewriting the fixture record. | Data scope appears correct; row presentation repair awaits deployed retest; filter labels remain |
| Fixture list/calendar buttons visibly switch views | On deployed build `b03938d`, the owner accepted the calendar colours and general presentation. Add previous/next month controls, retain historic rounds in the calendar and use a different colour for past fixtures. | Visual pass; month navigation and historic-state treatment logged |
...
| Completed fixture detail should show scraped result, participants and match statistics | For completed fixtures, show the final result plus the players who participated and their scraped goals/cards. In Team Availability, order regular team players who played first, participating fill-ins second and all other eligible players afterwards. Preserve each availability response and clearly distinguish regular/fill-in, played/did not play, available-but-not-selected and played-without-a-response. | Change logged — detailed workflow/design deferred |
