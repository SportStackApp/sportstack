...
| Scheduled fixture detail and availability controls work for actual Team Manager | Commit `df5b0ec` fixed the scheduled-detail crash and Aaron confirmed it displays. Follow-up testing found the unselected Maybe option unreadable, the toast called it maybe while the button called it Unsure, and a selected response could not be cleared. A local continuation uses **Maybe** consistently on Dashboard and Fixture Detail, adds readable unselected colours and matches the existing Dashboard click-again-to-clear behaviour. | **Detail load passed; availability UX continuation awaiting deployment/retest** |
