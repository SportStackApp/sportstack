...
| Communications | Team Chat, replies, reactions and removed placeholders loaded. An unsent draft survived in-app navigation away/back but was lost on a full reload. The edited legacy message opened an empty revision dialog. Notification click-through opened the correct fixture and reduced unread count from 3 to 2. Only seven messages existed, so earlier-batch pagination was not exercisable. No message/update was published. | **Partial pass — refresh draft, legacy history and pagination remain** |
...
| Umpire Match Voting | Actual Umpire login and ballot route passed. Candidate logic still loads all association profiles and memberships, assigning a fixture team only when the person belongs to one of the two teams. This does not meet the agreed rule of the two fixture teams plus recorded participants/fill-ins. No ballot was submitted in this continuation. | **Actual-role route pass — suggestion scope defect remains** |
...
| Safety Hub and Committee | Actual Association Admin retesting loaded every Safety Hub dashboard/register/guidance/audit tab and the guided Add Risk form without a write. Committee calendar, Polls, restricted Chat, Minutes and every administration tab loaded. The calendar shows a past `Test meeting` while Meetings says none are scheduled; confirm whether that wording intentionally means future meetings. Linked-record edits, uploads and other write paths remain unverified. | **Actual-role read-only pass; write paths and meeting wording remain — 09/08/2026** |
...
| Dev integrity and quality | Commit `e38150d` passed focused fixture lint, TypeScript, build, 16 Vitest tests, 148 Python tests, Dev Quality and Vercel deployment. Full lint remains its current baseline of 360 errors/78 warnings. Production dependency audit reports 10 high and one moderate advisory. Dev Supabase has 16 ACTIVE Edge Functions and its latest 100 Edge requests were HTTP 200, while advisors retain 69 security WARN and 239 performance WARN notices. | **Regression checks pass; lint/dependency/advisor debt parked — 09/08/2026** |
...
| All assigned teams appear, including Lucas HC | A temporary second Team Manager assignment for Lucas HC was added to the disposable Pumas Team Manager account. After a fresh login, the top bar stayed fixed on Grampians Hockey Club / Pumas and exposed no club switcher. The exact Lucas role row was then removed and verified absent. | **Fail — multi-club scope switch missing on Dev — 09/08/2026** |
