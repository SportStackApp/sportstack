...
| Coaching, roster, formation and profile | Squad/Roster deduplicated visible cards and showed relationship KPIs; Formation Builder panel collapse, focus mode and landscape surface worked; Team Player Details and role/scope profile sections loaded. Source review confirms the daily workflow is implemented through My Dashboard → fixture detail → Line-up: a player sets availability, a coach/manager sees roster availability, assigns or suggests the pitch, saves and publishes, and a linked player can open the published line-up. Squad/Roster are reference views rather than the selection screen. Cards still commonly lack positions/numbers. Line-up access currently reads stored roles directly and does not apply the active Viewing-as mode, so a higher-role account can retain edit access while testing a lower mode. | **Partial pass — actual-role publish workflow pending; active-mode restriction fails code review** |
