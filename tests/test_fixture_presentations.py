...
class FixturePresentationTests(unittest.TestCase):
    def test_calendar_mode_has_a_real_month_grid(self) -> None:
        ...
        self.assertIn("selectedTeamId={selectedTeamId}", source)
        ...
        self.assertIn('result === "win"', source)
        self.assertIn('border-green-300 bg-green-100', source)
        self.assertIn('result === "loss"', source)
        self.assertIn('border-red-300 bg-red-100', source)
        self.assertIn('result === "draw"', source)
        self.assertIn('border-orange-300 bg-orange-100', source)
        self.assertIn("getSelectedTeamScore", source)
        self.assertIn("{resultLabel} {score}", source)
