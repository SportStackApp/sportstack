...
| Coordination Module | Docs-only specification added at `docs/coordination-module-discovery.md`; no code, schema or live data changed. It defines scoped permissions, configurable fixture/activity positions, multi-person offers, deadlines/reminders, recipient notes, atomic acceptance, history and Umpire Match Voting roster checks. | Aaron to decide the offer-selection rule, default deadline/reminder timing and mismatch reviewer. Then recheck the live schema and prepare a migration/RLS test plan before implementation. |
