...
| Team Manager scheduled fixture detail | Actual Team Manager testing on deployed `f3486b0` passed the scoped Pumas fixture list but opening a scheduled fixture crashed. The local fix replaces mismatched `UNSURE`/legacy `PENDING` availability keys with generated enum values `MAYBE`/`NO_RESPONSE` and makes the style map exhaustive. All focused checks, build and 146 Python tests pass. | Commit/push to Dev, wait for quality/deployment, then reopen the same Pumas fixture as the actual Team Manager. No migration. |
