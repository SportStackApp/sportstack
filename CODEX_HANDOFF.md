...
| Coordination Module | Docs-only specification added at `docs/coordination-module-discovery.md`; no code, schema or live data changed. Several Umpires may accept a multi-person offer, but acceptance records willingness only. The original offerer must explicitly confirm one accepted Umpire before an official assignment exists. The design also covers deadlines/reminders, recipient notes, history and Umpire Match Voting roster checks. | Aaron to decide the default deadline/reminder timing and mismatch reviewer. Then recheck the live schema and prepare a migration/RLS test plan before implementation. |
