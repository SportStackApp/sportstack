...
export function CommitteeMeetings({ committeeId, associationId, clubId, canManage, canRecordMinutes, profiles }: { committeeId: string; associationId: string; clubId: string | null; canManage: boolean; canRecordMinutes: boolean; profiles: Array<{ id: string; first_name: string | null; last_name: string | null }> }) {
  ...
  const loadMeetings = useCallback(async () => {
    const [templateResult, templateItemResult, meetingResult, meetingItemResult, riskResult, actionResult, qiResult, ideaResult] = await Promise.all([
    ...
    const failure = [templateResult, templateItemResult, meetingResult, meetingItemResult, riskResult, actionResult, qiResult, ideaResult].find((result) => result.error)?.error;
    if (failure) {
    ...
    setMinuteDrafts(Object.fromEntries(loadedItems.map((item) => [item.id, {
      ...
      linkedRecordKey: item.linked_record_type && item.linked_record_id ? `${item.linked_record_type}:${item.linked_record_id}` : "",
    }])));
  ...
  const saveMinutes = async (item: MeetingItem) => {
    ...
    if (!draft) return;
    const [linkedRecordType, linkedRecordId] = draft.linkedRecordKey.split(":");
    setSaving(true);
