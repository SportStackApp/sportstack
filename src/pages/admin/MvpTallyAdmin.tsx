...
import { ArrowLeft, ArrowRight, Check, Eye, ImageOff, Palette, Send, Trophy, Upload, Users } from "lucide-react";
...
export default function MvpTallyAdmin() {
  ...
  const buildPreview = async () => {
    ...
    try {
      ...
      setPreview({ cards: result.cards, results: result.results, commentary: rules });
      setStep(3);
    } catch (error) {
  ...
  const publish = async () => {
    ...
    setBusy(true);
    try {
      await publishMvpTally(draftId);
      toast({
        title: "Tally published",
        description: "Selected players have received an in-app notification.",
      });
      setDraftId(null);
      setPreview(null);
      setStep(0);
    ...
    <div className="container mx-auto max-w-7xl space-y-6 p-4 md:p-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          {step === 4 && (
            <div className="mx-auto max-w-2xl space-y-5"><Alert><Send className="h-4 w-4" /><AlertTitle>Ready to publish</AlertTitle><AlertDescription>{selectedRoundOptions.length} rounds · {selectedAudienceCount} selected players. Publishing sends one in-app notification to each selected player. No email is sent.</AlertDescription></Alert><Button className="w-full bg-blue-700 text-white hover:bg-blue-800" disabled={busy} onClick={() => void publish()}>{busy ? "Publishing…" : "Publish now"}</Button></div>
          )}
