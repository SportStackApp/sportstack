  ...
  return (
    <div className="space-y-6">
      ...
      <SimpleDialog open={memberOpen} onOpenChange={setMemberOpen} title="Appoint committee member" description="Assign a user to a position with appointment dates." saving={saving} onSave={() => void saveMember()}>
        ...
        <div className="grid gap-3 sm:grid-cols-2"><InputField label="Start date" type="date" value={memberForm.startDate} onChange={(startDate) => setMemberForm((current) => ({ ...current, startDate }))} /><InputField label="End date (optional)" type="date" value={memberForm.endDate} onChange={(endDate) => setMemberForm((current) => ({ ...current, endDate }))} /></div>
